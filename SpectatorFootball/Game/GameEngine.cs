...
namespace SpectatorFootball.GameNS
{
    ...
    public class GameEngine
    {
            ...
            string KO_Type, long Injuries, long Penalties)
        {
            ...
            logger.Debug("Away");
            foreach (Player_and_Ratings p in Away_Players)
        ...
        public Play_Struct ExecutePlay()
        {
            ...
            if (!r.bForfeitedGame)
            {
                ...
                iPlay Play = null;
                if (Offensive_Package.Play == Play_Enum.KICKOFF_NORMAL || Offensive_Package.Play == Play_Enum.KICKOFF_MODERN)
                ...
                if (Offensive_Package.Play == Play_Enum.KICKOFF_AFTER_SAFETY)
                    Play = new Play_Kickoff_Free_Kick(Offensive_Package.Formation, DEF_Formation, g_fid_posession, at.Franchise_ID, ht.Franchise_ID, Game_Ball, Offensive_Players, Defensive_Players, bLefttoRight, false);
                ...
                if (Play.isAccumeStats() && p_result.Penalty != null && !p_result.bPenalty_Rejected && !p_result.bIgnorePenalty)
                {
            ...
            string pos_team_name_after_play = g_fid_posession == at.ID ? at.Nickname : ht.Nickname;

            logger.Debug("just before setting announcement");
            ...
            Tuple<List<string>, List<string>, List<string>> t = ann.getPlayResult_Announcement(Home_Players, Away_Players, ht.Nickname, at.Nickname, Offensive_Package.Play, p_result, bpreSnapPenalty,
                new_injury, Down_and_Yards, Game_Engine_Helper.getYardline(g_Line_of_Scrimmage), r.bForfeitedGame, pos_team_name_after_play);

            logger.Debug("BPO Test Announcements");
            if (p_result.Returner != null)
                logger.Debug("Returned to yl: " + p_result.Returner.Current_YardLine);
            if (p_result.Penalized_Player != null)
                logger.Debug("Pen Player yl: " + p_result.Penalized_Player.Current_YardLine);
            logger.Debug("Final ending yardline: " + p_result.Final_end_of_Play_Yardline);
            logger.Debug("isBallTeamPenalty: " + isBallCarryingTeam);

            logger.Debug(string.Join(" ", t.Item1));
            logger.Debug("");
            logger.Debug(string.Join(" ", t.Item2));
            logger.Debug("");
        ...
        public static Play_Result setPlayOutCome(bool penOnBallCarryingTeam,
            ...
            bool bLefttoRgiht, double TouchBack_Yardline)
        {
            ...
            switch (PE)
            {
                ...
                case Play_Enum.KICKOFF_ONSIDES:
                    ...
                    //if the kick is returned then flip lefttoright
                    if (r.bKick_Returned)
                        bLefttoRgiht = !bLefttoRgiht;

                    //Switch the ball possessing team on certain plays and situations
                    tp = setBAllPossessionTeam(pResult);
                    pResult.BallPossessing_Team_Id = tp.Item1;
                    pResult.NonbBallPossessing_Team_Id = tp.Item2;
                    ...
                    else if (r.Penalty == null || r.bPenalty_Rejected || (r.Penalty.bSpot_Foul && !penOnBallCarryingTeam))
                    {
                        ...
                        {
                            if (!pResult.Penalty.bDeclinable && pResult.bTouchDown)
                            {
                                r.bIgnorePenalty = true;
                ...
                case Play_Enum.FIELD_GOAL:
                    //Note Field Goals do not have spot fouls.
                    //this is why we don't check for spot fouls and penalties are not ignored.
                    if (r.Penalty != null && r.Penalty.bSpot_Foul)
                        throw new Exception("Field Goals can not have spot fouls");
                    ...
                    {
                        bool bignorePenalty = false;
                        setDefenseNonSpotPenalty(bignorePenalty, r, bLefttoRgiht, Down, Yards_to_Go);
                    }
                    else if (penOnBallCarryingTeam)
                    {
                ...
                case Play_Enum.EXTRA_POINT:
                    ...
                    {
                        bool bignorePenalty = false;
                        if (!pResult.Penalty.bDeclinable && pResult.bXPMade)
                            bignorePenalty = true;
                ...
                case Play_Enum.PUNT:
                    ...
                    else if (r.Penalty.bSpot_Foul && penOnBallCarryingTeam)
                    {
                        ...
                        double lesser_yl = Game_Engine_Helper.LessYardline(r.end_of_play_yardline, r.Penalized_Player.Current_YardLine, !bLefttoRgiht);
                        dist_from_GL = Game_Engine_Helper.calcDistanceFromMyGL(lesser_yl, !bLefttoRgiht);
                ...
                case Play_Enum.PASS:
                    ...
                    if (pResult.Penalty == null || pResult.bPenalty_Rejected || pResult.Penalty.bSpot_Foul)
                    {
                        ...
                        {
                            dist_from_GL = Game_Engine_Helper.calcDistanceFromOpponentGL(end_yardline, bLefttoRgiht);
                            bool bIgnorePeanlty = IgnorePenaltyRun_or_Pass(r);
                        ...
                        if (pResult.Penalty != null && pResult.Penalty.bSpot_Foul && penOnBallCarryingTeam)
                        {
                            ...
                            else
                            {
                                ...
                                if (PE == Play_Enum.PASS && pResult.bPassComplete)
                                {
                        ...
                        if (bTurnover || r.bSafety)
                        {
                            ...
                            if (bTurnover) setTurnoverGameStat(r);
                        }
        ...
        private static Play_Result setSacks(Play_Result pR)
        {
            if (pR.BallPossessing_Team_Id == pR.at)
            {
                pR.HomeSacks++;
                pR.HomeSackYards = pR.Yards_Gained;
            }
            else
            {
                pR.AwaySacks++;
                pR.AwaySackYards = pR.Yards_Gained;
            }
        ...
        private static void setDefenseNonSpotPenalty(Play_Result r, bool bLefttoRgiht, int Down, double Yards_to_Go)
        {
            ...
            {
                double dist_from_GL = Game_Engine_Helper.calcDistanceFromOpponentGL(r.Play_Start_Yardline, bLefttoRgiht);
                Tuple<bool, double> t = Penalty_Helper.isHalfTheDistance(r.Penalty.Yards, dist_from_GL);
                ...
                r.Final_end_of_Play_Yardline = r.Play_Start_Yardline + (Penalty_Yards * Game_Engine_Helper.HorizontalAdj(bLefttoRgiht));
                r.Final_Added_Penalty_Yards = Penalty_Yards;
            }
        }
        private static void setDefenseNonSpotPenalty(bool bIgnorePeanlty, Play_Result r, bool bLefttoRgiht, int Down, double Yards_to_Go)
        {
            bool bHalft_the_dist = false;
            double Penalty_Yards;
        ...
        {
            double Penalty_Yards;
            double dist_from_GL = Game_Engine_Helper.calcDistanceFromMyGL(r.Play_Start_Yardline, bLefttoRgiht);
            Tuple<bool, double> t = Penalty_Helper.isHalfTheDistance(r.Penalty.Yards, dist_from_GL);
            if (t.Item1)
                Penalty_Yards = t.Item2;
            else
        ...
        public static Play_Result setScoringBool(Play_Result r)
        {
            ...
            long BallPossessing_Team = pResult.BallPossessing_Team_Id;

            if (r.bInterception || r.bFumble_Lost)
            {
                BallPossessing_Team = BallPossessing_Team == pResult.at ? pResult.ht : pResult.at;
            }

            if (BallPossessing_Team == pResult.at)
            {
        ...
        public static Play_Result setRushPassYards(Play_Result r, long Original_Ball_Possessing_Team)
        {
            Play_Result pResult = r;

            if (Original_Ball_Possessing_Team == pResult.at)
            {
                if (pResult.Receiver != null)
                    r.AwayPassingYards = r.Yards_Gained;
                else if (pResult.Running_Back != null)
                    r.AwayRushingYards = r.Yards_Gained;
            }
            else
            {
                if (pResult.Receiver != null)
                    r.HomePassingYards = r.Yards_Gained;
                else if (pResult.Running_Back != null)
                    r.HomeRushingYards = r.Yards_Gained;
        ...
        private static bool IgnorePenaltyRun_or_Pass(Play_Result pResult)
        {
            bool r = false;

            if ((pResult.Penalty.code == Penalty_Codes.FM ||
                pResult.Penalty.code == Penalty_Codes.UC ||
                pResult.Penalty.code == Penalty_Codes.UR) &&
                (pResult.bTouchDown || pResult.bSafety))
                r = true;
        ...
        private static Tuple<long, long> setBAllPossessionTeam(Play_Result pResult)
        {
            long BallPossessingTeam = pResult.BallPossessing_Team_Id;
            long NonBallPossessingTeam = pResult.NonbBallPossessing_Team_Id;

            //for a punt, switch the ball possessing team as technically the returners are
            //considered the ball possessiong team
            if (pResult.Returner != null || pResult.Punt_Returner != null ||
                pResult.bInterception || pResult.bFumble_Lost)
            {
                BallPossessingTeam = pResult.at == pResult.BallPossessing_Team_Id ? pResult.ht : pResult.at;
                NonBallPossessingTeam = BallPossessingTeam == pResult.at ? pResult.ht : pResult.at;
            }
        ...
        {
            double r = Normal_TB;
            if (pe == Play_Enum.KICKOFF_NORMAL || pe == Play_Enum.KICKOFF_MODERN || pe == Play_Enum.KICKOFF_DYNAMIC || pe == Play_Enum.KICKOFF_AFTER_SAFETY || pe == Play_Enum.KICKOFF_ONSIDES)
                r = kickoff_TB;
