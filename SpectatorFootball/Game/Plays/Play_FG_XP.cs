...
namespace SpectatorFootball.GameNS
{
    public class Play_FG_XP : iPlay
    {
            ...
            List<Game_Player> FG_Players, List<Game_Player> FG_Def_Players, Play_Result pResult)
        {
            ...
            {
                if (p != r.Kicker)
                    r.FieldGaol_Kicking_Team.Add(p);
        ...
        private void Kick_blocked(Game_Ball gBall, List<Game_Player> FG_Players, List<Game_Player> FG_Def_Players, Game_Player blocker, bool bLefttoRight)
        {
            ...
            foreach (Game_Player p in FG_Def_Players)
            {
                ...
                if (p == blocker)
                    ...
                    p.Delay_Then_Run_and_Stand(moving_ps, prev_yl, prev_v, 15);
                io_Players++;
            }
            ...
            switch (t.Item3)
            {
                ...
                case FG_Path.HIT_GOALPOST_INTO_CROWD:
                    ...
                    if (!pr.bFGMade && !pr.bXPMade)
                        hit_gp_crowd_adj *= -1;
                    gBall.FG_Hits_GP(prev_yl, prev_v, t.Item6, t.Item7, true, bLefttoRight, hit_gp_crowd_adj);
                    break;
                case FG_Path.HIT_GAOLPOST:
                    if (!pr.bFGMade && !pr.bXPMade)
                        hit_gp_crowd_adj *= -1;
                    pr.bFGXPHitGP = true;
                    gBall.FG_Hits_GP(prev_yl, prev_v, t.Item6, t.Item7, false, bLefttoRight, hit_gp_crowd_adj);
                    break;
            }
