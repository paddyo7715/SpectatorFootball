...
{
    /// <summary>
    /// Interaction logic for Stock_Team_detail.xaml
    /// </summary>
    public partial class Stock_Team_detail : UserControl
    {

        private Stock_Teams binding_stock_team = new Stock_Teams();
        private Stock_Teams orig_this_team = null;
        ...
        public Stock_Team_detail(Stock_Teams this_team)
        {
            // This call is required by the designer.
            InitializeComponent();
            if (this_team != null)
                binding_stock_team = Team_Helper.ClonseStock_Team(this_team);

            this.DataContext = binding_stock_team;
            ...
            else
            {
                ...
                all_uniform_colors = Uniform.getAllColorList(
                    binding_stock_team.Helmet_Color,
                    binding_stock_team.Helmet_Logo_Color,
                    binding_stock_team.Helmet_Facemask_Color,
                    binding_stock_team.Socks_Color,
                    binding_stock_team.Cleats_Color,
                    binding_stock_team.Home_jersey_Color,
                    binding_stock_team.Home_Sleeve_Color,
                    binding_stock_team.Home_Jersey_Number_Color,
                    binding_stock_team.Home_Jersey_Number_Outline_Color,
                    binding_stock_team.Home_Jersey_Shoulder_Stripe,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_1,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_2,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_3,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_4,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_5,
                    binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_6,
                    binding_stock_team.Home_Pants_Color,
                    binding_stock_team.Home_Pants_Stripe_Color_1,
                    binding_stock_team.Home_Pants_Stripe_Color_2,
                    binding_stock_team.Home_Pants_Stripe_Color_3,
                    binding_stock_team.Away_jersey_Color,
                    binding_stock_team.Away_Sleeve_Color,
                    binding_stock_team.Away_Jersey_Number_Color,
                    binding_stock_team.Away_Jersey_Number_Outline_Color,
                    binding_stock_team.Away_Jersey_Shoulder_Stripe,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_1,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_2,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_3,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_4,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_5,
                    binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_6,
                    binding_stock_team.Away_Pants_Color,
                    binding_stock_team.Away_Pants_Stripe_Color_1,
                    binding_stock_team.Away_Pants_Stripe_Color_2,
                    binding_stock_team.Away_Pants_Stripe_Color_3);
            }
        ...
        private void Stock_Team_detail_Loaded(Object sender, EventArgs e)
        {
        ...
        private void newtbtnHelmetImgPath_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (OpenFileDialog.ShowDialog() == true)
            {
                ...
                binding_stock_team.Helmet_img_path = Path.GetFileName(filepath);
        ...
        private void newtbtnStadiumPath_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                string filepath = OpenFileDialog.FileName;
                binding_stock_team.Stadium_Img_Path = Path.GetFileName(filepath);
        ...
        private void newt1Add_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (bnew_team)
                    ...
                    st.UpdateStockTeam(binding_stock_team);

                backtoStockTeams?.Invoke(this, new EventArgs());
            }
            catch (Exception ex)
            {
                logger.Error("Error saving stock team error");
                logger.Error(ex);
        ...
        private void Validate()
        {
                ...
                if (CommonUtils.isBlank(binding_stock_team.City_Abr))
                    throw new Exception("City Abbriviation must have a value");

                if (!CommonUtils.isAlpha(binding_stock_team.City_Abr, false))
                    throw new Exception("Invalid character in City Abbriviation!");

                if (CommonUtils.isBlank(binding_stock_team.City))
                ...
                if (!Validator.isValidateColorString(binding_stock_team.Stadium_Field_Color))
                    throw new Exception("Field color of team stadium must be supplied");

                if (CommonUtils.isBlank(binding_stock_team.Stadium_Capacity))
                    throw new Exception("Stadium Capacity must be supplied and numeric");

                if (CommonUtils.isBlank(binding_stock_team.Stadium_Img_Path))
                    throw new Exception("Image of team stadium must be supplied");

                if (!Validator.isValidateColorString(binding_stock_team.Helmet_Color))
                    throw new Exception("A helmet color must be selected");

                if (!Validator.isValidateColorString(binding_stock_team.Helmet_Logo_Color))
                    throw new Exception("A helmet Logo color must be selected");

                if (!Validator.isValidateColorString(binding_stock_team.Helmet_Facemask_Color))
                    throw new Exception("A helmet facemask must be selected");

                if (CommonUtils.isBlank(binding_stock_team.Helmet_img_path))
                    throw new Exception("Helmet image path must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Socks_Color))
                    throw new Exception("Sock color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Cleats_Color))
                    throw new Exception("Cleat color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_jersey_Color))
                    throw new Exception("Home jersey color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Sleeve_Color))
                    throw new Exception("Home sleeve color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Shoulder_Stripe))
                    throw new Exception("Home shoulder loop color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Number_Color))
                    throw new Exception("Home jersey number color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Number_Outline_Color))
                    throw new Exception("Home jersey outline number color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_1))
                    throw new Exception("Home jersey sleeve string 1 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_2))
                    throw new Exception("Home jersey sleeve string 2 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_3))
                    throw new Exception("Home jersey sleeve string 3 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_4))
                    throw new Exception("Home jersey sleeve string 4 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_5))
                    throw new Exception("Home jersey sleeve string 5 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Jersey_Sleeve_Stripe_Color_6))
                    throw new Exception("Home jersey sleeve string 6 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Pants_Color))
                    throw new Exception("Home pants color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Pants_Stripe_Color_1))
                    throw new Exception("Home pants stripe 1 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Pants_Stripe_Color_2))
                    throw new Exception("Home pants stripe 2 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Home_Pants_Stripe_Color_3))
                    throw new Exception("Home pants stripe 3 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_jersey_Color))
                    throw new Exception("Away jersey color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Sleeve_Color))
                    throw new Exception("Away sleeve color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Shoulder_Stripe))
                    throw new Exception("Away shoulder loop color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Number_Color))
                    throw new Exception("Away jersey number color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Number_Outline_Color))
                    throw new Exception("Away jersey outline number color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_1))
                    throw new Exception("Away jersey sleeve string 1 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_2))
                    throw new Exception("Away jersey sleeve string 2 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_3))
                    throw new Exception("Away jersey sleeve string 3 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_4))
                    throw new Exception("Away jersey sleeve string 4 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_5))
                    throw new Exception("Away jersey sleeve string 5 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Jersey_Sleeve_Stripe_Color_6))
                    throw new Exception("Away jersey sleeve string 6 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Pants_Color))
                    throw new Exception("Away pants color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Pants_Stripe_Color_1))
                    throw new Exception("Away pants stripe 1 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Pants_Stripe_Color_2))
                    throw new Exception("Away pants stripe 2 color must have a value");

                if (!Validator.isValidateColorString(binding_stock_team.Away_Pants_Stripe_Color_3))
                    throw new Exception("Away pants stripe 3 color must have a value");

                // Check if stock team is a duplicate by city and nickname.
                if (bnew_team)
                {
                    var st_services = new StockTeams_Services();
                    if (st_services.DoesTeamAlreadyExist(binding_stock_team.City.Trim(), binding_stock_team.Nickname.Trim()))
                        throw new Exception("This Stock Team Already Exists!  No Duplicate Stock Teams Allowed!");
                }
                else if (binding_stock_team.City.Trim().ToUpper() != orig_this_team.City.ToUpper() || binding_stock_team.Nickname.Trim().ToUpper() != orig_this_team.Nickname.ToUpper())
                {
                    var st_services = new StockTeams_Services();
                    if (st_services.DoesTeamAlreadyExist(binding_stock_team.City.Trim(), binding_stock_team.Nickname.Trim()))
                        throw new Exception("This Stock Team Already Exists!  No Duplicate Stock Teams Allowed!");
